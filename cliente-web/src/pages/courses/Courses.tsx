...
import { getCoursesByTitle } from "../../hooks/api";
...
const Courses: React.FC = () => {
    const renderCourseCards = (courses: CourseCardInterface[]) => {
        return courses.map((course, index) => {
            return (
                <CourseCard key={index} {...course} redirectsOnClick={true} />
            );
        });
    };
    const handleTitleChange: React.ChangeEventHandler<HTMLInputElement> = (
        ...
        if (newValue === "") setCourses(getCoursesByTitle());
        setCourseTitle(event.target.value);
    };
    const handleSubmit: React.FormEventHandler = (event) => {
        event.preventDefault();
        setCourses(getCoursesByTitle(courseTitle));
    };
    const [courseTitle, setCourseTitle] = useState("");
    const [courses, setCourses] = useState(getCoursesByTitle());
